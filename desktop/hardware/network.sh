#!/usr/bin/env bash
# Make NetworkManager the sole network manager (retire archinstall's networkd
# state that competes with NM). Derived from Omarchy install/hardware/network.sh.
...
backup_dir="/etc/systemd/network/veloguard-networkd-retired-$(date +%Y%m%d%H%M%S)"
